...
def HasRotated(logfile, hash):
    ...
    was last executed."""
    timestamp = utcnow()
    cursor.execute("SELECT hash, date FROM rotate WHERE logfile = ?", (logfile,))
    ...
    if not result:
        ...
        cursor.execute(
            "INSERT INTO rotate (logfile, hash, date) VALUES (?, ?, ?)",
            (logfile, hash, timestamp),
        )
        con.commit()
        return timestamp
    if result[0] == hash:
    ...
    cursor.execute(
        "UPDATE rotate SET hash = ?, date = ? WHERE logfile = ?",
        (hash, timestamp, logfile),
    )
    con.commit()
    return timestamp
...
def DB_Exists(logfile, timestamp):
    ...
    logfile."""
    cursor.execute(
        "SELECT hit FROM master WHERE logfile = ? AND date = ?",
        (logfile, timestamp),
    )
...
def DB_Insert(hit, logfile, count, timestamp):
    """Insert a new entry using bound parameters.

    Values originate in remotely supplied article/log data.  SQLite parameters
    keep data separate from SQL syntax and also avoid fragile hand-escaping.
    """
    cursor.execute(
        "INSERT INTO master (hit, logfile, count, date) VALUES (?, ?, ?, ?)",
        (hit, logfile, count, timestamp),
    )
    con.commit()
...
def DB_Update(hit, logfile, count, timestamp):
    cursor.execute(
        """UPDATE master SET count = ?
           WHERE hit = ? AND logfile = ? AND date = ?""",
        (count, hit, logfile, timestamp),
    )
    con.commit()
...
def DB_Delete(hit):
    count = cursor.execute("DELETE FROM master WHERE hit = ?", (hit,)).rowcount
    con.commit()
    return count


def DB_Expire(hours):
    threshold = hours_ago(hours)
    cursor.execute(
        "SELECT COUNT(hit) FROM master WHERE datetime(date) < datetime(?)",
        (threshold,),
    )
    expire = cursor.fetchone()
    cursor.execute(
        "DELETE FROM master WHERE datetime(date) < datetime(?)",
        (threshold,),
    )
    con.commit()
    return expire[0]
...
def File2List(filename):
    ...
    items = []
    if IsFile(filename):
        readlist = open(filename, 'r', encoding='utf-8', errors='replace')
...
def ScanFiles(files):
    ...
    from_regex = re.compile(r'From foo@bar', re.M)
    ...
    for filename in files:
        if IsFile(filename):
            ...
            if GetSize(filename) < config.hashlength:
                print("File too small to hash, ignoring.")
                continue
            ...
            file.seek(0)
            hash = hashlib.sha1(fingerprint.encode('utf-8', errors='surrogateescape')).hexdigest()
...
def ProcessLogfiles():
    ...
    badurl = open(config.textfile, 'w', encoding='utf-8')
